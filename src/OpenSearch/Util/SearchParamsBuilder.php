...
class SearchParamsBuilder
{
    ...
    {
        $config = new Config();
        $this->searchParams = new SearchParams(array(
            'config' => $config
        ));
    ...
    {
        $this->searchParams->config->appNames = is_array($appNames) ? $appNames : array(
            $appNames
        );
    }
